...
db.serialize(() => {
  ...
  logDebug('Database initialized with all required columns', 'db');
});
...
app.post('/register', async (req, res) => {
  ...
  if (!/^[А-Яа-яЁёA-Za-z\s]+$/.test(father_name) || 
      !/^[А-Яа-яЁёA-Za-z\s]+$/.test(first_name) || 
      !/^[А-Яа-яЁёA-Za-z\s]+$/.test(address)) {
  ...
  try {
    ...
    // Handle optional passport_reg_number
      ...
      try {
        ...
        if (user.passport_reg_number) {
          try {
            ...
            // Ignore passport_reg_number if it cannot be decrypted
